...
a = arduino();
   ...
   % Initialize arrays to store time data and temprature data
    ...
    while true
        ...
        temp_C = (temp_raw - 0.5) * 100; % Convert to temperature in °C (adjust formula based on sensor)
        ...
        elseif temp_C < 18
            ...
            writeDigitalPin(a, red_led, 0);
            writeDigitalPin(a, yellow_led, mod(time_counter, 2)); % Blink every second
        else
            % Red LED blinking
            writeDigitalPin(a, green_led, 0);
            writeDigitalPin(a, yellow_led, 0);
            writeDigitalPin(a, red_led, mod(time_counter, 2)); % Blink every 0.5 second
        end
